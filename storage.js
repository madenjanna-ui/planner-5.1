...
function getTasks(date){if(!tasks[date])tasks[date]=[];return tasks[date]}
function addTask(date,text,options={}){const item={text,done:false,priority:"normal",created:Date.now(),recurrenceId:options.recurrenceId||null};getTasks(date).push(item);saveStorage();return item}
function addRecurringTask(startDate,text,type,until){const id="r_"+Date.now()+"_"+Math.random().toString(36).slice(2,7);const start=new Date(startDate+"T12:00:00");const end=new Date((until||new Date(start.getFullYear()+1,start.getMonth(),start.getDate()).toISOString().slice(0,10))+"T12:00:00");let d=new Date(start);let guard=0;while(d<=end&&guard<500){const date=localDateKey(d);addTask(date,text,{recurrenceId:id});if(type==="daily")d.setDate(d.getDate()+1);else if(type==="weekly")d.setDate(d.getDate()+7);else if(type==="weekdays"){do{d.setDate(d.getDate()+1)}while(d<=end&&(d.getDay()===0||d.getDay()===6))}else if(type==="monthly"){d.setMonth(d.getMonth()+1)}else break;guard++}appData.recurrences.push({id,startDate,text,type,until:end.toISOString().slice(0,10)});saveStorage()}
...
function editTask(date,index,text){if(!tasks[date])return;tasks[date][index].text=text;saveStorage()}
